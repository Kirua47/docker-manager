...
export default function VolumesPage() {
  ...
  return (
    <div className="flex flex-col gap-lg animate-in fade-in duration-700">
      ...
      {isCreating && (
        <form onSubmit={handleCreate} className="bg-surface border border-outline-variant rounded-2xl p-lg shadow-lg shadow-surface-container-low/50 transition-all origin-top max-w-2xl">
          <h3 className="font-title-sm text-title-sm text-on-surface mb-lg">New Volume Configuration</h3>
          <div className="space-y-lg">
            <div className="w-full">
              <label className="block font-body-sm text-body-sm text-on-surface-variant mb-xs font-medium">Volume Name</label>
              <input 
                type="text" 
                placeholder="e.g., db_data_production" 
                className="w-full bg-surface-container-low border border-outline-variant focus:border-primary/50 focus:bg-surface-container-lowest outline-none font-body-md text-body-md text-on-surface px-md py-3 rounded-xl transition-all placeholder:text-outline/70 focus:ring-2 focus:ring-primary/25"
                value={newVolumeName}
                onChange={(e) => setNewVolumeName(e.target.value)}
                autoFocus
              />
              <p className="text-xs text-on-surface-variant/70 mt-xs">Use alphanumeric characters, underscores, or hyphens.</p>
            </div>
            <div className="flex justify-end gap-md pt-sm">
              <button 
                type="button" 
                onClick={() => setIsCreating(false)}
                className="bg-surface-container-low hover:bg-surface-container-high border border-outline-variant font-title-sm text-title-sm py-3 px-lg rounded-xl transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button 
                ...
                className="bg-primary text-on-primary font-title-sm text-title-sm py-3 px-lg rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 font-semibold cursor-pointer"
              >
                Create Volume
              </button>
            </div>
